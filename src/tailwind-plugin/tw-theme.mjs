...
const calculateFontSizes = (base, scale) => {
  ...
  // Calculate raw sizes first (h6 to h1)
  const rawSizes = {};
  for (let i = 6; i >= 1; i--) {
    rawSizes[`h${i}`] = currentSize;
    currentSize *= scale;
  }

  // Apply custom multipliers from your v3 system
  sizes["h1"] = `${rawSizes["h1"] * 1.6667}rem`;
  sizes["h1-sm"] = `${rawSizes["h1"] * 0.9}rem`;

  sizes["h2"] = `${rawSizes["h2"] * 1.525}rem`;
  sizes["h2-sm"] = `${rawSizes["h2"] * 0.9}rem`;

  sizes["h3"] = `${rawSizes["h3"] * 1.4242}rem`;
  sizes["h3-sm"] = `${rawSizes["h3"] * 0.9}rem`;

  sizes["h4"] = `${rawSizes["h4"] * 1.2857}rem`;

  sizes["h5"] = `${rawSizes["h5"] * 1.1739}rem`;

  sizes["h6"] = `${rawSizes["h6"] * 1.1053}rem`;

  // Base sizes
  sizes.base = `${base}px`;
  sizes["base-sm"] = `${base * 0.8}px`;

  sizes.p = `${base * 1.13}px`;
  sizes["p-sm"] = `${base * 0.8}px`;
